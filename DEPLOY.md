...
Release gate: `npm test` must be **27/27 with `CHROME_PATH` set**. A SKIP on test-network.js is not a pass - that test is the only thing proving the consent gate still makes the tool inert, and it is the check most worth having before a visual release.
